...
In any given state, the attention matrix can be visualized in the following way: we connect the ego-vehicle to every each vehicle by a line with a width proportional to the corresponding attention weight. Since the architecture can contain several Ego-Attention heads, we use different colours to distinguish them. In our experiments, we used two attention heads represented in green and blue. We observe that both heads exhibit a common behaviour: they focus their attention on incoming vehicles that are likely to collide with the ego-vehicle, depending on their current position, heading, velocity, and ignore those that are too far or in a conflict-less situation. However, we can also notice that the two heads have specialized to focus on different areas: the green head is only watching the vehicles coming from the left, while the blue head restricts itself to vehicles in the front and right directions.
...
\begin{figure}
	...
	\begin{minipage}{.45\textwidth}
		...
		\captionof{figure}{Specialization of the attention heads}
		\label{fig:heads}
	\end{minipage}
\hfill
	\begin{minipage}{.5\textwidth}
		...
		\captionof{figure}{Generally, the attention increases as the vehicles get closer.}
		\label{fig:distance}
	\end{minipage}
\end{figure}
...
\begin{figure}[tp]
	...
	\end{subfigure}
	\caption{Sensitivity to uncertainty. \textbf{Right}: the agent has stopped at the intersection, its attention is focused on an incoming vehicle whose destination is still uncertain. \textbf{Left}: as soon as the incoming vehicle changes orientation, revealing a right turn, the attention drops and the agent starts accelerating.}
...
We trained a policy for left-turns, which is the hardest one as it had both the most conflict points and the lowest priority level. We also trained individual policies for right turns and straight crossings, and obtained similar results. When training a joint intersection policy, it is less efficient as it has no prior information on where the ego-vehicle is going. 
A generic intersection could be trained, but additional information must be added as additional features in \eqref{eq:coordinates}. For instance, the destination can be encoded as a desired direction $d_x, d_y$. This destination feature could also be used for other vehicles to encode blinker information when available. This should result in a more efficient policy.

\section{Conclusion}

In this work, we showed that the \emph{list of features} representation, commonly used to describe vehicles in autonomous driving literature, is not tailored for use in a function approximation setting, in particular with neural networks. These concerns can be addressed by the \emph{spatial grid} representation, but the latter comes at the price of an increased input size and loss of accuracy. In contrast, we proposed an attention-based neural network architecture to tackle the aforementioned issues without compromising either size or accuracy. This architecture enjoys a better performance on a simulated negotiation and intersection crossing task. 
