...
%Descriptive features, names keys $k_i$
%The similarity of the query $q_i$ to the keys $k_j$ is assessed through a similarity measure implemented as a scalar product: $<q_i, k_j>$, $QK^T$.
%These similarities are then normalized by a softmax $\sigma$ function and a division by $\sqrt{d_k}}$ where $d_k$ is the dimension of keys and queries. The final value is obtained.
...
Ego attention: only the ego-vehicle produces a query, we get one output feature. We can easily see that the result is permutation invariant: indeed, a permutation $\tau$ will change the order of the rows of keys $K$ and values $V$ but will keep their correspondence. The final result is a sum of values weighted by their key-similarities, which is independent of the ordering. %$\sigma\left(Q\tau(K)^T/\sqrt{d_k}\right)\tau(V) = \tau\left(\sigma(QK^T/\sqrt{d_k})\right)\tau(V) = \sigma\left(QK^T/\sqrt{d_k}\right)V$
